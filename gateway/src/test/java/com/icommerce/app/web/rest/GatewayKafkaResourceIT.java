...
import com.icommerce.app.config.KafkaProperties;
...
class GatewayKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
